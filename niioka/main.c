...
int set_pwm_output(int fd, int pwmch, int outval) {
 ...
 if (ef == 0) {
   regno = PWM_0_ON_L + pwmch * 4; // 1chあたり4レジスタで16ch分ある
   if (outval == 16) {
...
int main() {
 ...
 // コースに置いたらスタート
 while (1) {
   if (digitalRead(GPIO_L) == LOW && digitalRead(GPIO_R) == LOW)
     break;
 }

 int ms, ls, rs;
 int not_read_count = 0; // 真っ白な場所のカウント
 while (1) {
   ms = 0;
   ls = 0;
   rs = 0;
   if (digitalRead(GPIO_L) == HIGH) {
     printf("right\n");
     rs = 5;
     not_read_count = 0; // 真っ白でない場所にいるのでカウントをリセット
     ...
     printf("not_read\n");
     not_read_count++; // 真っ白な場所のカウントをインクリメント

     // 真っ白な場所に一定時間以上いた場合、左に曲がるようにする
     if (not_read_count > 10) {
       ls = 5; // 左に曲がるパワーを設定
       rs = -5; // 右に曲がるパワーを設定
     }
   }
